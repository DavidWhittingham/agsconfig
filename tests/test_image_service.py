...
def test_load_imager_server_config(service_config):
...
@pytest.mark.parametrize( #yapf:disable
    ...
    [
        ...
        (
            ...
            [
                ImageServer.MosaicMethod.north_west,
...
def test_image_server_getters(service_config, attribute, expected_value, exception):
...
@pytest.mark.parametrize(
    ("attribute", "new_value", "expected_value", "exception"), [
        ...
        ("capabilities", "Something", "Something", ValueError), ("client_caching_allowed", False, False, None),
        ...
        ("cache_dir", "a:/cache/dir", "a:/cache/dir", None), ("client_caching_allowed", False, False, None),
        ("credits", "Credits", "Credits", None), ("description", "Description", "Description", None),
        ("folder", "a:/folder", "a:/folder", None), ("high_isolation", False, False, None),
        ("idle_timeout", 1200, 1200, None), ("instances_per_container", 2, 2, None), ("keep_cache", True, True, None),
        ("max_image_height", 410, 410, None), ("max_image_width", 1500, 1500, None), ("max_instances", 2, 2, None),
        ("max_scale", 1000, 1000, None), ("max_record_count", 2000, 2000, None), ("min_instances", 2, 2, None),
        ("min_scale", 10, 10, None), ("name", "Example", "Example", None),
        ("raster_functions", ["None", "Path/To/RasterFunction.xml"], ["None", "Path/To/RasterFunction.xml"], None),
        ("recycle_interval", 1, 1, None), ("recycle_start_time", datetime.time(16, 0), datetime.time(16, 0), None),
        ("summary", "summary", "summary", None), ("tags", ["blah"], ["blah"], None),
        ("title", "SomeTitle", "SomeTitle", None), ("usage_timeout", 6000, 6000, None),
        ("wait_timeout", 160, 160, None), ("has_valid_sr", False, False, None),
        ...
        ("available_fields", ["ONE", "TWO"], ["ONE", "TWO"], None), ("return_jpgpng_as_jpg", True, True, None),
        ("max_mosaic_image_count", 10, 10, None), ("max_download_size_limit", 10, 10, None),
        ("max_download_size_limit", -1, None, ValueError), ("max_download_size_limit", None, None, None),
        ("max_download_image_count", 20, 20, None), ("max_download_image_count", -1, None, ValueError),
        ("max_download_image_count", None, None, None), ("has_valid_sr", True, True, None)
    ]
)
def test_image_server_setters(service_config, attribute, new_value, expected_value, exception):
    if exception is not None:
        with pytest.raises(exception):
