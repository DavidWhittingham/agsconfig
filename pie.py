...
__all__ = [
    'task', 'Parameter', 'OptionsParameter', 'options', 'cmd', 'cd', 'env',
    'pip', 'venv'
]
...
class TaskWrapper(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        defaults = dict(
            zip(arg_names[len(arg_names) - len(defaults):],
        ...
        for param in self.params:
            if param.name not in arg_names:
                raise Exception('{} not a valid parameter of task {}'.format(
                    param.name, self.fn.__name__))
    ...
    def getValue(self, default):
        ...
        if default is Parameter.NO_VALUE or self.use_default == False:
            ...
            if promptStr is None:
                defaultStr = ' (default: {!r})'.format(
                    default) if default is not Parameter.NO_VALUE else ''
                promptStr = 'Please enter a value for {}{}: '.format(
...
class venv(CmdContext):
    ...
    def create(self, extraArguments='', pythonCmd=None, py3=PY3):
        ...
        c = r'"{}" -m {} {} "{}"'.format(pythonCmd, venv_module,
                                         extraArguments, self.path)
        cmd(c)
    ...
    def cmd(self, c):
        ...
        if WINDOWS:
            ...
            c = r'''cmd /c ""{}" && {}"'''.format(
                self._binary_path('activate.bat'), c)
        else:
            c = r'bash -c "source "{}" && {}"'.format(
                self._binary_path('activate'), c)
        return CmdContextManager.cmd(c, self.contextPosition)
    ...
    def _get_sys_prefix(self):
        ...
        long_name_prefix = u'\\\\?\\'
        sys_prefix = sys_prefix if sys_prefix.startswith(
            long_name_prefix) else u'{}{}'.format(long_name_prefix, sys_prefix)

        # find out how long the long name path is
        sys_prefix_chars = windll.kernel32.GetLongPathNameW(
            sys_prefix, None, 0)
        ...
        if sys_prefix_chars:
            ...
            sys_prefix_long_name_buffer = create_unicode_buffer(
                sys_prefix_chars)
            ...
            if windll.kernel32.GetLongPathNameW(sys_prefix,
                                                sys_prefix_long_name_buffer,
                                                sys_prefix_chars):
        ...
        e = GetLastError()
        error_template = u'Failed to get long name for sys.prefix ({}): {{}}'.format(
            sys.prefix)
        if e:
            formatted_error = FormatError(e).decode(
...
class env(CmdContext):
    ...
    def enter_hook(self):
        self.old_env_dict = self.get_multiple(self.env_dict.keys(),
                                              default=None)
...
class Help(Argument):
    def execute(self):
        print(
            'Usage:    pie [ -V | -h | -b | -l | -L | -m <name> | -R | -r | -n | -v ]'
        )
        ...
        print(
            '  -r      Updates the .venv-pie venv using requirements.pie.txt')
        ...
        print('')
        print(
            'The order of -o and <task> options matters - each will be executed in the order given on the command line.'
        )
...
class TaskCall(Argument):
    ...
    def __repr__(self):
        return 'Task: {}(args={},kwargs={})'.format(self.name, self.args,
                                                    self.kwargs)
    ...
    while i < len(args):
        ...
        if arg.startswith('-'):
            ...
            if arg == '-V':
                ...
                if '=' not in args[i + 1]:
                    raise Exception(
                        'Option ("{}") must be in format name=value'.format(
                            args[i + 1]))
    ...
    if parsed_args:
        ...
        if any([a.needsTasksImported for a in parsed_args]):
            ...
            if not pv.requirements_exists() or pv.is_activated():
                ...
                if pv.exists():
                    ...
                    print(
                        '{} not found. You can create it with the -R argument.'
                        .format(pv.PIE_VENV),
                        file=sys.stderr)
                    return 1
        ...
        for a in parsed_args:
            try:
                ...
                print(
                    'Error when executing command "{}". Errorcode = {}'.format(
                        e.cmd, e.errorcode),
                    file=sys.stderr)
                return e.errorcode
